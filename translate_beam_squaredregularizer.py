...
def get_args():
    ...
    # The lambda for the weights
    parser.add_argument('--λ', default=0.5, type=float, help='lambda for weights')
    ...
    for i, sample in enumerate(progress_bar):
        ...
        for i in range(batch_size):
            for j in range(args.beam_size):
                ...
                regularizer = torch.sum(log_p ** 2)

                # apply it to formula
                regularized_log_p = log_p - args.λ * regularizer

                node = BeamSearchNode(searches[i], emb, lstm_out, final_hidden, final_cell,
                                      mask, torch.cat((go_slice[i], next_word)), regularized_log_p, 1)
        ...
        for _ in range(args.max_len-1):
            ...
            for i in range(log_probs.shape[0]):
                for j in range(args.beam_size):
                    ...
                    # Apply regularizer to formula
                    regularized_log_p = log_p - args.λ * regularizer

                    # Check if EOS is generated and store the node as final
                    ...
                    # Add the node to current nodes for next iteration
                        ...
                        node = BeamSearchNode(
                            search, node.emb, node.lstm_out, node.final_hidden,
                            node.final_cell, node.mask, torch.cat((prev_words[i][0].view([1]), next_word)),
                            node.logp + regularized_log_p, node.length + 1
                        )
