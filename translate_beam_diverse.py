...
def main(args):
    ...
    # ------------------ START OF NEW CODE FOR ASSIGNMENT 5 -----------------
    # Call the adapted beam method
    top_n_hypotheses = [search.get_best_n(args.n_translations) for search in searches]

    top_translations = []
    for top_hypotheses in top_n_hypotheses:
        translations = []
        for rank, node in enumerate(top_hypotheses):
            sent = node[1].sequence[1:].cpu().numpy() #node[1] is necessary as we are dealing with tuples
            first_eos = np.where(sent == tgt_dict.eos_idx)[0]
            if len(first_eos) > 0:
                sent = sent[:first_eos[0]]
            sentence = tgt_dict.string(sent)
            translations.append(sentence)
        top_translations.append(translations)

    for ii, translations in enumerate(top_translations):
        all_hyps[int(sample['id'].data[ii])] = translations

    # Sort dictionary according to keys:
    all_hyps_sorted = {key: all_hyps[key] for key in sorted(all_hyps)}

    # Write to file
    if args.output is not None:
        with open(args.output, 'w') as out_file:
            for key, sentences in all_hyps_sorted.items():
                for sentence in sentences:
                    out_file.write("(" + str(key) + ") " + sentence + '\n')
