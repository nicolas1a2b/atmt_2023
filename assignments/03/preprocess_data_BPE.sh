...
python3 assignments/03/bpe_tokenizer.py "data/en-fr/raw/train.fr" > $data/preprocessedBPE/train.$src.p
python3 assignments/03/bpe_tokenizer.py "data/en-fr/raw/train.en" > $data/preprocessedBPE/train.$tgt.p

# normalize and tokenize raw data
:'
cat $data/raw/train.$src | perl moses_scripts/normalize-punctuation.perl -l $src | python3 assignments/03/bpe_tokenizer.py "$1" > $data/preprocessedBPE/train.$src.p
cat $data/raw/train.$tgt | perl moses_scripts/normalize-punctuation.perl -l $tgt | python3 assignments/03/bpe_tokenizer.py "$2" > $data/preprocessedBPE/train.$tgt.p
